...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const result = await authFetch("/auditlogs?page=1&pageSize=20");

    if (!result || !result.items || result.items.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="text-muted text-center py-4">No audit logs recorded.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = result.items.map(a => `
        <tr>
          ...
          <td><span class="badge bg-primary">${escapeHtml(a.action)}</span></td>
          <td>${escapeHtml(a.entityType)}</td>
          <td>${escapeHtml(a.entityId || 'N/A')}</td>
