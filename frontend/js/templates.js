...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    tbody.innerHTML = '<tr><td colspan="5" class="text-muted text-center py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading message templates...</td></tr>';

    const templates = await authFetch("/templates");

    if (!templates || templates.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-muted text-center py-4">No active communication templates found.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = templates.map(t => `
        <tr>
          <td>${t.templateID}</td>
          <td class="fw-bold">${escapeHtml(t.templateName)}</td>
